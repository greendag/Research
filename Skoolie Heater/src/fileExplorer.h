...
const char *ssid = "UncleSlappy";
const char *password = "GrannyDot#5274774";
...
void Directory(String folder = "/")
{
    ...
    File root = FS.open(folder.c_str());
    if (root)
    {
        Serial.println("root is valid.");
...
void Dir(AsyncWebServerRequest *request)
{
    ...
    if (numfiles > 0)
    {
        ...
        while (index < numfiles)
        {
            ...
            {
                webpage += "<td style = 'width:5%'>" + Filenames[index].ftype + "</td><td style = 'width:25%'><a href='/dir?folder=" + Fname1 + "'>" + Fname1 + "</a></td><td style = 'width:10%'>" + Filenames[index].fsize + "</td>";
            }
            else
            {
            ...
            if (index < numfiles - 1)
            {
                ...
                {
                    webpage += "<td style = 'width:5%'>" + Filenames[index + 1].ftype + "</td><td style = 'width:25%'><a href='/dir?folder=" + Fname2 + "'>" + Fname2 + "</a></td><td style = 'width:10%'>" + Filenames[index + 1].fsize + "</td>";
                }
                else
                {
...
void Select_File_For_Function(AsyncWebServerRequest *request, String title, String function)
{
    ...
    while (index < numfiles)
    {
        ...
        webpage += "<tr>";
        webpage += "<td style='width:25%'><button><a href='" + function + "~" + rootFolder + Fname1 + "'>" + Fname1 + "</a></button></td><td style = 'width:10%'>" + Filenames[index].fsize + "</td>";
        webpage += "<td class='sp'></td>";
        if (index < numfiles - 1)
        {
            webpage += "<td style='width:25%'><button><a href='" + function + "~" + rootFolder + Fname2 + "'>" + Fname2 + "</a></button></td><td style = 'width:10%'>" + Filenames[index + 1].fsize + "</td>";
        }
        webpage += "</tr>";
        index = index + 2;
    }
...
void fileExplorerSetup()
{
    ...
    Select_File_For_Function(request, "[STREAM]", "streamhandler"); // Build webpage ready for display
    request->send(200, "text/html", webpage); });
    ...
    Select_File_For_Function(request, "[DELETE]", "deletehandler"); // Build webpage ready for display
    request->send(200, "text/html", webpage); });
